...
class LegacyWorkflowNode:
    node_id = "legacy_workflow"
    phase = 100
    order = 100

    def apply(self, state: ComfyWorkflowState) -> None:
        state.set_workflow(_build_legacy_workflow(state.context))
...
def register_nodes(registry: ComfyNodeRegistry) -> None:
    registry.register(LegacyWorkflowNode())


def _build_legacy_workflow(context: ComfyWorkflowBuildContext) -> dict[str, Any]:
    ...
    if models.controlnet_name and context.reference_image_name:
        control_loader_inputs = context.required_input_defaults("ControlNetLoader")
        control_name_field = context.select_field_name(
        ...
        if control_name_field:
            ...
            control_image_id = _node("LoadImage", {"image": context.reference_image_name})

            apply_class = ""
            if "ControlNetApplyAdvanced" in context.object_info:
                apply_class = "ControlNetApplyAdvanced"
            elif "ControlNetApply" in context.object_info:
                apply_class = "ControlNetApply"

            if apply_class:
                control_apply_inputs = context.required_input_defaults(apply_class)
                pos_field = context.select_field_name(
                    control_apply_inputs,
                    ("positive", "conditioning"),
                )
                neg_field = context.select_field_name(
                    control_apply_inputs,
                    ("negative",),
                )
                controlnet_field = context.select_field_name(
                    control_apply_inputs,
                    ("control_net", "controlnet"),
                )
                image_field = context.select_field_name(
                    control_apply_inputs,
                    ("image", "hint", "control_image"),
                )
                strength_field = context.select_field_name(
                    control_apply_inputs,
                    ("strength",),
                )
    ...
    if context.reference_mode == "ipadapter" and context.reference_image_name:
        apply_class = context.info.ipadapter_apply_node
        if not apply_class:
            raise RuntimeError("IP-Adapter apply node not available")

        clip_loader_inputs = context.required_input_defaults("CLIPVisionLoader")
        clip_name_field = context.select_field_name(
            clip_loader_inputs,
            ("clip_name", "model_name"),
        )
        if not clip_name_field or not context.info.clip_vision_models:
            raise RuntimeError("CLIP Vision loader is not configured")
        clip_loader_inputs[clip_name_field] = context.info.clip_vision_models[0]
        ...
        ip_model_loader_inputs = context.required_input_defaults("IPAdapterModelLoader")
        ip_model_field = context.select_field_name(
            ip_model_loader_inputs,
            ("ipadapter_file", "model_name", "ipadapter_name"),
        )
        if not ip_model_field or not context.info.ipadapter_models:
            raise RuntimeError("IP-Adapter model loader is not configured")
        ip_model_loader_inputs[ip_model_field] = context.info.ipadapter_models[0]
        ...
        ref_image_id = _node("LoadImage", {"image": context.reference_image_name})

        apply_inputs = context.required_input_defaults(apply_class)
        model_field = context.select_field_name(apply_inputs, ("model",))
        image_field = context.select_field_name(apply_inputs, ("image", "images"))
        clip_field = context.select_field_name(
            apply_inputs,
            ("clip_vision", "clipvision"),
        )
        ipadapter_field = context.select_field_name(
            apply_inputs,
    ...
    if context.reference_mode == "img2img" and context.reference_image_name:
        load_image_id = _node("LoadImage", {"image": context.reference_image_name})
        if enhancements.enable_tiled_diffusion:
            latent_id = _node(
                "VAEEncodeTiled",
                {
    ...
    skip_base_sampling = context.skip_base_sampler_pass and enhancements.enable_hires_fix
